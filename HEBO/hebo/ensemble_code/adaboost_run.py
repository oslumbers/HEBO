...
if __name__ == '__main__':
    ...
    for i in range(seeds):
        space_cfg = [
                {'name' : 'n_estimators',         'type' : 'int', 'lb' : 10, 'ub' : 100},
                {'name' : 'learning_rate',      'type' : 'pow', 'lb' : 1e-4, 'ub' : 10},
                ]
        
        result, report = tuner(AdaBoostClassifier, space_cfg, X, y, metric = mean_squared_error, greater_is_better=False, max_iter = 3, num_ens=args.num_ens)
        print(f'result: {result}')
        seed_res.append(report)
    # Dump the report to a pickle file
    save_dir = f'results_testing/adaboost/{args.dataset}'
